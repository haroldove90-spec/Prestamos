...
export const AdminHeader: React.FC<AdminHeaderProps> = ({ currentUser, onUserChange, onResetData, onToggleSidebar }) => {
  ...
  return (
    <header className="bg-[#a3c90e] border-b border-[#8dae09] text-white shadow-md relative z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 md:py-3.5 flex items-center justify-between gap-2 min-h-[56px]">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          {onToggleSidebar && (
            <button
              ...
              className="lg:hidden p-1.5 sm:p-2 rounded-lg bg-white/20 hover:bg-white/30 text-white transition active:scale-95 cursor-pointer flex items-center justify-center shrink-0"
          ...
          {/* Icon Badge */}
          <div className="hidden sm:flex p-2 rounded-xl bg-white/15 text-white border border-white/10 shrink-0">
            <ShieldCheck className="w-5 h-5" />
          </div>

          {/* Titles */}
          <div className="text-left leading-tight">
            <h1 className="text-sm sm:text-lg font-extrabold tracking-tight text-white flex items-center gap-1.5 font-sans">
              Buró Interno de Crédito
              <span className="text-[9px] sm:text-[10px] font-bold font-mono px-1.5 sm:px-2 py-0.5 rounded-full bg-slate-950/40 text-white uppercase border border-white/10 shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-3 text-[11px] font-mono text-white/90">
          <div className="flex items-center gap-1.5 bg-slate-955 bg-slate-950/20 px-3 py-1 rounded-full border border-white/10">
            <Calendar className="w-3.5 h-3.5 text-white" />
            <span>Sesión: 20 de Mayo, 2026</span>
          </div>
          <div className="flex items-center gap-1.5 bg-slate-950/20 px-3 py-1 rounded-full border border-white/10">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <div className="hidden md:flex gap-1 bg-slate-950/15 p-1 rounded-xl border border-white/15">
            <button
          ...
          <button
            ...
            className="bg-white/15 hover:bg-white/25 text-white font-mono text-[10px] md:text-xs px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg border border-white/10 transition cursor-pointer shrink-0"
          >
            Reset
          </button>
          ...
          <div className="relative shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 p-1 rounded-full hover:bg-white/15 transition duration-150 cursor-pointer focus:outline-none"
              title="Abre el selector de perfiles"
            >
              {/* White Circle with Deep Teal border - matches the mock visual element precisely */}
              <div className="w-8 h-8 rounded-full border-2 border-white bg-[#0a3a46] text-[#a3c90e] font-extrabold flex items-center justify-center shadow-md transform hover:scale-105 active:scale-95 transition-all duration-150 text-xs sm:text-sm">
